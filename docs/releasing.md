...
The fork's inherited upstream SemVer tags were audited before the independent release line was reopened. The 43 inherited original tags are preserved as `upstream-<original-tag>` refs and removed from the fork's original namespace. `v1.0.0`, `v3.3.7-enhanced.1`, `upstream-v1.0.0`, and `komari` remain protected; `v1.0.1` is available for the Enhanced release.
