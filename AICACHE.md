...
- 当前验证：Preview 替换后 `bun run lint`、`bun run type-check`、`bun run build`、`bun run test:visual` 52/52、`git diff --check` 均通过；新增 history/Tooltip/安全用例全部通过。PR #2 的 Code Quality 与 Visual Regression 对应 `f726c5a` 均已远端成功；最终交接提交后需重新读回 CI。远端结果不能用本地结果替代。
